# <div align='center'>FanzzModzz</div>

<p align="center">

  <img src="https://files.catbox.moe/5kdspi.jpg" width="180" alt="FanzzModzz"/>
...
A WebSockets library for interacting with WhatsApp Web, maintained under the **FanzzModzz** brand.
...
```bash
npm install github:FanzzModzz/Baileys
```

Or add it to your `package.json` manually:
```json
"dependencies": {
  "@FanzzModzz/baileys": "github:FanzzModzz/Baileys"
}
```
...
```bash
npm install github:FanzzModzz/Baileys#main
```
...
const client = makeWASocket({
  browser: Browsers.FanzzModzz('Chrome'),
...
await client.relayMessage(m.chat, {
  conversation: "Hello from FanzzModzz"
}, {
  noSelfSync: true
});
...
```javascript
const fs = require('fs');
const thumbnail = fs.readFileSync('./Fanzzthumb.jpg');
...
await client.sendMessage(m.chat, {
  pollResultMessage: {
    ...
    newsletter: {
      newsletterName: "Zenith - Comunity",
      newsletterJid: "1203634249440@newsletter"
    }
  }
});
```
...
## Send image
```javascript
await client.sendImage(m.chat, { url: "./Fanzzimg.jpg" }, "Caption", {
  contextInfo: {
    mentionedJid: [m.chat]
  }
}, {
...
## Send audio
```javascript
await client.sendAudio(m.chat, { url: "./Fanzzaudio.mp3" }, {
  contextInfo: {
    mentionedJid: [m.chat]
  }
}, {
...
FanzzMidzz is a fork of [Baileys](https://github.com/WhiskeySockets/Baileys), originally created by
